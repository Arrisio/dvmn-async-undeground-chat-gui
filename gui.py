...
async def update_conversation_history(panel, messages_queue, fix_vbar_treshold=.999):
    while True:
        ...
        if panel.yview()[1] > fix_vbar_treshold:
            panel.yview(tk.END)
...
async def draw(chat_queues):
    ...
    input_field.bind("<Return>", lambda event: process_new_message(input_field, chat_queues.sending_queue))

    send_button = tk.Button(input_frame)
    send_button["text"] = "Отправить"
    send_button["command"] = lambda: process_new_message(input_field, chat_queues.sending_queue)
